"""macOS dialog backend using tkinter."""
...
class TkDialogs(DialogBackend):
    """tkinter-based dialogs for macOS."""
    ...
    def show_settings(self) -> Optional[dict[str, str]]:
        ...
        mount_entry = ttk.Entry(frame, textvariable=mount_var, width=30)
        mount_entry.grid(row=row, column=1, sticky="ew")
        ...
        error_label.grid(row=row, column=0, columnspan=3, sticky="w", pady=(8, 0))
        row += 1
    ...
    def show_logs(self, text: str) -> None:
        ...
        st = scrolledtext.ScrolledText(root, wrap=tk.WORD, font=("Courier", 11))
        st.pack(fill="both", expand=True)
        st.insert("1.0", text)
        st.config(state="disabled")
        st.see("end")

        ttk_frame = tk.Frame(root)
        ttk_frame.pack(fill="x", padx=8, pady=8)
        tk.Button(ttk_frame, text="Close", command=root.destroy).pack(side="right")
